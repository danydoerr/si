...
if __name__ == '__main__':
    ...
    if not ((options.absolute > 0) ^ (options.scale_factor > 0)):
        print >> stderr, '!! Specify either scale factor or absolute length with strictly positive number'
        exit(1)

    for tree in parse_tree_iterator(open(args[0])):
        if options.absolute > 0:
            print rescale_absolute(tree, options.absolute)
        else:
            print rescale(tree, options.scale_factor)
